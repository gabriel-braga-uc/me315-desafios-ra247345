...
dbListTables(db)
```
Em geral, DROP TABLE é perigoso. Se você abrir seu servidor, pode ficar
sujeito aos chamados "injection attacks"!

## Um ligeiro parêntese:Bestpractices

Esse exemplo está na documentação em https://db.rstudio.com: se você tem
um aplicativo (por exemplo, em Shiny) que colhe inputs do usuário em aname
e diz quais álbums deste artista estão listados:
...
```{r}
# Antes,riando de volta a tabela de instrumentos
dbExecute(db,
"CREATE TABLE instruments
(AlbumId INTEGER,
TrackId INTEGER,
...
## Inserindo um atabela diretamente
...
```
## Inserindo uma tabela direta mente: overwrite
...
## ...mas secretamente, sãocomandos de SQLite!
